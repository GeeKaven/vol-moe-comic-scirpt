...
fn suffix_change(in_type: &str, out_type: &str, path: &str) -> Result<()> {
...
fn classfy_comic(path: &str, cache: &str) -> Result<()> {
...
fn pack_comic(cache: &str, output: &str) -> Result<()> {
...
fn main() {
    // 开始重命名文件
    match suffix_change("epub", "zip", "./dist") {
        Ok(_) => println!("OK"),
        Err(err) => println!("Err: {}", err),
    }
    //开始提取图片
    match classfy_comic(PATH, CACHE_FOLDER) {
        Ok(_) => println!("提取OK"),
        Err(err) => println!("Err: {}", err),
    }
    // 开始打包图片
    match pack_comic(CACHE_FOLDER, OUTPUT_FOLDER) {
        Ok(_) => println!("打包OK"),
        Err(err) => println!("Err: {}", err),
    }
    // 清理换成文件
